...
export class EditHomeComponent implements OnInit {
  ...
  getMainCategoriesList() {
    this._products.getMainCategoriesListAdmin().subscribe(
      (res) => {
        //console.log(res)
        this.categoryList = [];
        this.products = [];
  ...
  getMainCategoryProducts(category) {
    this._products.getMainCategoryProducts(category).subscribe(
      (res) => {
  ...
  addMainCategoriesTitle() {
    this._products.addMainCategoriesTitle(this.categoryDetails).subscribe(
      (res) => {
        console.log(res);
        this.categoryList = [];
        this.products = [];
  ...
  addMainCategoryProducts() {
    this._products.addMainCategoryProducts(this.prodcutDetails).subscribe(
      (res) => {
        console.log(res);
        this.products = [];
        for (let i = 0; i < this.categoryList.length; i++) {
  ...
  removeCategoryFromHome(category) {
    ...
    this._products.removeCategoryFromHome(category).subscribe(
      (res) => {
        console.log(res);
        this.getMainCategoriesList();
      },
  ...
  removeProductFromHome(id) {
    console.log(id);
    this._products.removeProductFromHome(id).subscribe(
      (res) => console.log(res),
      (err) => console.log(err)
    );
  }
  ...
  changeMainCategoryVisibilty(event: any, category) {
    if (confirm("Are you sure want to enable/disable ? ")) {
      console.log(category);
      this._products.changeMainCategoryVisibilty(category).subscribe(
        (res) => {
