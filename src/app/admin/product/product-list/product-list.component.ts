...
export class ProductListComponent implements OnInit {
  ...
  getPermision() {
    this._auth.getPermisionUser().subscribe(
      (res) => {},
      (err) => {
  ...
  getAllProducts() {
    this._product.getAllProducts().subscribe(
      (res) => {
        console.log(res), (this.productList = res);
      },
      (err) => {
        console.log(err);
        if (err instanceof HttpErrorResponse) {
  ...
  changeProductVisibilty(event: any, id) {
    if (confirm("Are you sure want to enable/disable ? ")) {
      this._product.changeProductVisibilty(id).subscribe(
        (res) => {
  ...
  changeProductDiscountOnOff(event: any, id) {
    if (confirm("Are you sure want to enable/disable ? ")) {
      this._product.changeProductDiscountOnOff(id).subscribe(
        (res) => {
          this.toggleClass2(event, res);
        },
        (err) => console.log(err)
