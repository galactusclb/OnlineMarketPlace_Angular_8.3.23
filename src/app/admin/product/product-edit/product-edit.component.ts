...
export class ProductEditComponent implements OnInit {
  ...
  getProductDetails(id) {
    this._product.getProductDetails(id).subscribe(
      (res) => {
        console.log(res), (this.productDetails = res[0]);
        this.imgURL =
          this.url + "/uploads/products/" + this.productDetails["pic"];
  ...
  updateProdcutsDetails() {
    this._product.updateProdcutsDetails(this.productDetails).subscribe(
      (res) => console.log(res),
      (err) => console.log(err)
    );
  }
  updateProductTags() {
    ...
    this._product
      .updateProductTags(newTagList.join(","), this.productDetails["id"])
      .subscribe(
        (res) => {
  ...
  updateProductImg() {
    if (this.selectedFile) {
      ...
      this._product.updateProductImg(fd).subscribe(
        (event) => {
